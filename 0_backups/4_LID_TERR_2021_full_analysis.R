...
wd <- setwd("C:/Users/caleb vogt/Box/LID_TERR_2021/RFID and Analysis/ccv_analyses")
output_fp <- paste("C:/Users/Caleb Vogt/Desktop/output_fp")


# load data
meta <- read.csv("LID_TERR_2021_metadata_ID.csv",header=T)
...
data<-fread("LID_TERR_2021_ALLTRIAL_RFID_DATA.csv")
...
data<-subset(data,data$noon_to_noon_day<35)
...
# [] Within-tub Read Interval & Time window capture thresholds, histogram -----------------------------------------------
...
df <- data
df <- subset(data,data$noon_to_noon_day<=35)
df$Zone <- df$Antenna
ids <- unique(df$Name)
big_data_list <- list()
data_list <- list()
...
df3$time<-strftime(df3$Field_Time, format="%H:%M:%S")
df3$day_night<-ifelse(as.numeric(substr(df3$time,1,2))>=6&as.numeric(substr(df3$time,1,2))<18,"day","night")


sdat <- summary(df3$diff_secs)
sdat
## get time interval where 95% of intervals below that value. 
sort(subset(df3,day_night=="day"&Sex=="F")$diff_secs)[0.95*length(subset(df3,day_night=="day"&Sex=="F")$diff_secs)]  
sort(subset(df3,day_night=="night"&Sex=="F")$diff_secs)[0.95*length(subset(df3,day_night=="night"&Sex=="F")$diff_secs)]  #Use Female 95% during night; 960 seconds

sort(subset(df3,day_night=="day"&Sex=="M")$diff_secs)[0.95*length(subset(df3,day_night=="day"&Sex=="M")$diff_secs)]  
sort(subset(df3,day_night=="night"&Sex=="M")$diff_secs)[0.95*length(subset(df3,day_night=="night"&Sex=="M")$diff_secs)]  #Use Male 95% during the night; 233 seconds 


sort(subset(df3,day_night=="day"&Sex=="F")$diff_secs)[0.99*length(subset(df3,day_night=="day"&Sex=="F")$diff_secs)]  #Use Female 99% during day; 2295 seconds
sort(subset(df3,day_night=="night"&Sex=="F")$diff_secs)[0.99*length(subset(df3,day_night=="night"&Sex=="F")$diff_secs)]  

sort(subset(df3,day_night=="day"&Sex=="M")$diff_secs)[0.99*length(subset(df3,day_night=="day"&Sex=="M")$diff_secs)]  #Use Male 99% during the day; 990 seconds
sort(subset(df3,day_night=="night"&Sex=="M")$diff_secs)[0.99*length(subset(df3,day_night=="night"&Sex=="M")$diff_secs)]  

sort(df3$diff_secs)[0.95*length(df3$diff_secs)]  
sort(df3$diff_secs)[0.99*length(df3$diff_secs)]  
...
df <- as.data.frame(fread("T001_MOVEBOUT_GBI.csv", stringsAsFactors = TRUE)) ### CHANGE THIS
print(unique(df$trial))
el_list <- list()
df$field_time_Start <- as.POSIXct(df$field_time_Start, format="%Y-%m-%d %H:%M:%OS")
df$field_time_Stop <- as.POSIXct(df$field_time_Stop, format="%Y-%m-%d %H:%M:%OS")
df <- df[,order(df$field_time_Start)]
