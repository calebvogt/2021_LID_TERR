...
## note that this is not counting animals that did not meet any females. 
## use gbi_mf_encountered until fixed. 
...
m1 = lmer(num_f_met ~ num_terr*day + (1|focal), data = filter(df1, day %in% 1:5, num_terr %in% 1:2)) 
summary(m1)
AIC(m1)

m2 = lmer(num_f_met ~ num_terr*day + (1|focal), data = filter(df1, day %in% 6:20, num_terr %in% 1:2)) 
summary(m2)
AIC(m2)

m3 = lmer(num_f_met ~ num_terr*day + (1|focal), data = filter(df1, day %in% 6:20, num_terr %in% 0:2)) 
summary(m3)
AIC(m3)
