...
df <- move_data %>% 
  ...
  group_by(noon_to_noon_day, antenna) %>% 
  tally(sum(duration_min)) %>% 
  mutate(day_antenna = paste0(noon_to_noon_day, "_", antenna)) %>% 
  dplyr::rename(total_duration_min = n) %>% #specify dplyr due to conflict
  ungroup() %>% 
  dplyr::select(day_antenna, total_duration_min)
...
df1 <- move_data %>% 
  ...
  mutate(duration_min = duration_s / 60) %>% 
  mutate(day_antenna = paste0(noon_to_noon_day, "_", antenna)) %>% 
  group_by(name, day_antenna, noon_to_noon_day, antenna) %>% 
  tally(sum(duration_min)) %>%
  dplyr::rename(mus_duration_min = n)#specify dplyr due to conflict

df2 <- merge(df1, df, by = "day_antenna", all = TRUE) # bring in rest of males that did not win any days. 
df3 <- merge(df2, meta_short, by = "name", all = FALSE) # bring in metadata

df4 <- df3 %>%
  mutate(name_treatment = paste0(name,"_", treatment)) %>% 
  # filter(treatment == "early") %>% ## choose 
  filter(noon_to_noon_day %in% (1:20)) # first 20 days
...
df5 <- df4 %>% 
  ...
  group_by(name) %>%
  complete(noon_to_noon_day = 1:20, fill = list(penalty = -1, mus_percent_capture = 0)) %>% # PENALTY #2: Not observed at all penalty. ## CHANGE DAY
  ...
  select(!c(sex, code, treatment)) # remove this information so you can bring it back in next. 
...
# filter(!(treatment == "late" & noon_to_noon_day %in% (1:5))) ## Remove days 1-5
...
df8 <- df6 %>%
  ...
  mutate(label = if_else(noon_to_noon_day == max(noon_to_noon_day), as.character(name_treatment), NA_character_))
...
# line plot
library(ggrepel)
(p <- ggplot(late_pas, aes(x=noon_to_noon_day, y=sum_daily_capture_penalty, group = name_treatment, color = name_treatment)) + #y=csum_adj_mus_percent_capture_score
