...
df1 <- df %>% 
  ...
  summarise(mean = mean(node_betweeness_centrality), sd = sd(node_betweeness_centrality), count = n(), sem = (sd/(sqrt(count))))
...
ggplot(df1, aes(x=day, y=mean, color=as.factor(num_terr))) + 
  ...
  # scale_x_continuous(limits = c(0.8,5.3), breaks = seq(1, 10, by = 1)) +
    ...
    theme_classic() +
    xlab("Night") +
    ylab("Node betweenness centrality") +
  ...
  # legend.position = "")
ggsave(file="output/sna_node_betweeness_centrality.svg",device="svg",unit="in",width=3,height=3,bg = "transparent") 
